...
const onRefreshed = (token: string) => {
  refreshSubscribers.forEach((cb) => cb(token));
  refreshSubscribers = []; // clear sau khi gọi
};
...
// ====== Axios Instance for V1 (with refresh) ======
const apiAxios = axios.create({
  baseURL: "https://ec2api.deltatech-backend.com/api/v1",
});
...
// ====== Axios Instance for V2 (no refresh logic) ======
const apiAxiosV2 = axios.create({
  baseURL: "https://ec2api.deltatech-backend.com/api/v2",
});
